...
defimpl Core.Domain.Policies.SchedulingPolicy, for: Data.Configurations.AAPP do
  @moduledoc """
  ...
  @doc """
  Selects a suitable worker to host the given function, according to the provided AAPP configuration.

  ## Parameters
  - configuration: an AAPP script (Data.Configurations.AAPP), generally obtained through parsing using the Core.Domain.Policies.Parsers.AAPP module.
  ...
  """
  @spec select(AAPP.t(), [Data.Worker.t()], Data.FunctionStruct.t(), map()) ::
          {:ok, Data.Worker.t()} | select_errors()
  def select(config, workers, function, args \\ %{})

  def select(
        %AAPP{tags: tags} = _configuration,
        [_ | _] = workers,
  ...
  def select(%AAPP{tags: _}, [], _, _) do
    {:error, :no_workers}
  end

  def select(%AAPP{tags: _}, _, %Data.FunctionStruct{metadata: nil}, _) do
    {:error, :no_function_metadata}
  end
  ...
  @doc """
  Helper function, recursively explores the blocks specified for a function's tag in an AAPP configuration.

  ## Parameters
  - blocks: a list of AAPP blocks (Data.Configurations.AAPP.Block), extracted from the function's tag definition in the AAPP configuration.
  - workers: a map with String keys and Data.Worker values; the keys are each worker's "long_name" attribute, which are the same names used inside the AAPP configuration.
  - function: a Data.FunctionStruct struct, with the necessary function information. It must contain function metadata, specifically a tag and a capacity.
  ...
  - affinity: a list of strings representing affinity rules. Strings starting with "!" are antiaffinity rules.

  ## Returns
  - true if the worker satisfies all affinity requirements
  - false otherwise
  """
  @spec affinity_valid?(Data.Worker.t(), [String.t()]) :: boolean
  def affinity_valid?(_worker, []) do
    true
  end
